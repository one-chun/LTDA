...
def compute_train_degrees(edge_index, edge_attr, train_index, num_nodes):
	train_ids = torch.as_tensor(train_index, dtype=torch.long)
	link_degree = torch.zeros(num_nodes, dtype=torch.float)
	type_degree = torch.zeros(num_nodes, dtype=torch.float)

	if train_ids.numel() == 0:
		return link_degree, type_degree

	u = edge_index[0, train_ids]
	v = edge_index[1, train_ids]
	one = torch.ones_like(u, dtype=torch.float)
	relation_count = edge_attr[train_ids].sum(dim=1).float()

	link_degree.index_add_(0, u, one)
	link_degree.index_add_(0, v, one)
	type_degree.index_add_(0, u, relation_count)
	type_degree.index_add_(0, v, relation_count)
	return link_degree, type_degree
...
class PPIData(object):
	def __init__(self,args,class_num = 7):
		...
		# The paper describes concatenating sequence and structure features
		# before graph propagation; keep both raw views and expose the fused input.
		self.prot_input = torch.cat([self.prot_embed, self.prot_encode], dim=1)
		self.split_dict = {}
		if args.m == 'bfs':
		...
		edge_attr = torch.tensor(self.interList, dtype=torch.long)
		link_degree, type_degree = compute_train_degrees(
			edge_index2,
			edge_attr,
			self.split_dict['train_index'],
			self.seqsNum
		)

		sparse_adj2 = create_sparse_tensor(edge_index2,self.seqsNum)
		self.data = torch_geometric.data.Data(
			embed1=self.prot_input,
			encode1=self.prot_encode,
			struct_embed1=self.prot_embed,
			edge1=edge_index1,
			edge2=edge_index2,
			edge_attr=edge_attr,
			sparse_adj1=sparse_adj1,
			sparse_adj2=sparse_adj2
		) 
		self.data.train_mask = self.split_dict['train_index']
		self.data.val_mask = self.split_dict['valid_index']
		self.data.link_degree = link_degree
		self.data.type_degree = type_degree
